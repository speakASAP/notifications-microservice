...
```text
2026-07-02: Notifications deploy script was hardened to set the Kubernetes deployment image to the immutable image tag instead of `:latest`. This addresses repeated stale-pod/digest drift that blocked invoices documents readiness with HTTP 401 when an older image lacked `INVOICES_NOTIFICATIONS_SERVICE_TOKEN` support. Validation on `alfares`: `bash -n scripts/deploy.sh` passed, `npm run build` passed, focused `npm test -- --runInBand src/auth/jwt-roles.guard.spec.ts src/notifications/channel-registry.service.spec.ts src/notifications/notifications.service.spec.ts` passed, and `git diff --check` passed. Live deployment was already pinned to `localhost:5000/notifications-microservice:f855764`; no additional deploy, send, provider call, channel mutation, or customer contact was performed.
2026-07-02: Invoices documents Notifications runtime provisioning completed on `codex/notifications-orders-lifecycle-integration` at commit `a73df0b`. Vault key presence for `secret/prod/invoices-microservice#NOTIFICATIONS_SERVICE_TOKEN` was verified without printing the value; `k8s/external-secret.yaml` was applied and force-synced; `notifications-microservice-secret` now has `INVOICES_NOTIFICATIONS_SERVICE_TOKEN`; `channel_registry` has active `invoices.documents` policy with `type=email`, `provider=ses`, `purposesAllowed={transactional}`, and `applicationsAllowed={invoices-microservice}`. Pre-deploy validation on `alfares`: `git diff --check` passed, `npm run build` passed, and `npm test -- --runInBand` passed 7 suites / 32 tests. `./scripts/deploy.sh` built/pushed image digest `sha256:4e12aef822773d9ffec333db6417403ac6b5a73cf855ab8e25fb2bcb664f25a1`; because the deployment uses `:latest`, an explicit `kubectl rollout restart deployment/notifications-microservice -n statex-apps` was required. Final pod `notifications-microservice-64ff99b44f-jdvpx` is ready on that digest. `./scripts/check-invoices-documents-readiness.sh` passed with proforma/final HTTP 201, `mutation=false`, `providerCall=false`; smoke-recipient notification rows stayed `0 -> 0`. No real `/notifications/send`, provider dispatch, template persistence, webhook delivery, customer data mutation, or customer contact occurred.
